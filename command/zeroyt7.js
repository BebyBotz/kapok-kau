...
break
case 'play':
			if (!q) return m.reply(`Kirim perintah *${prefix}play* _Judul lagu yang akan dicari_`)
            var srch = args.join('')
   			...
   			var mulaikah = aramat[0].url							
                  try {
                    ...
                    .then((res) => {
                        ...
                        axios.get(`https://tinyurl.com/api-create.php?url=${dl_link}`)
                        .then(async (a) => {
                        if (Number(filesize) >= 100000) return sendFileFromUrl(m.chat, thumb, `*PLAY MUSIC*\n\n*Title* : ${title}\n*Ext* : MP3\n*Filesize* : ${filesize}\n*Link* : ${a.data}\n\n_Untuk durasi lebih dari batas disajikan dalam mektuk link_`)
                        const captions = `*PLAY MUSIC*\n\n*Title* : ${title}\n*Ext* : MP3\n*Size* : ${filesize}\n*Link* : ${a.data}\n\n_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
                        sendFileFromUrl(m.chat, thumb, captions)
                        await sendFileFromUrl(m.chat, dl_link).catch(() => m.reply('error'))
                        })                
                        })
                        } catch (err) {
                        m.reply(mess.eror)
...
case 'tiktok': case 'ttdl':
 if (!q) return m.reply('Linknya?')
            ...
            .then((data) => { sendFileFromUrl(m.chat, data.result.watermark,`Nieh By Aprilia\nJangan Lupa Subscribe Bot Indo`) })
